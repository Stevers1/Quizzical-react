...
function App() {
  ...
  function checkAnswers() {
    if (!checkedAnswers) {
      let countCorrect = formQuestion.reduce(
        (acc, obj) => (obj.isCorrect === true ? acc + 1 : acc),
        0
      );
      setDispalyScore(countCorrect);

      document
        .querySelectorAll(".question_container")
    ...
    <main className="main">
      {firstLogin && (
        <div className="intial_screen">
          ...
          <p>
            This app generates quiz questions to evaluate our knowledge made by{" "}
            <a href="https://github.com/Stevers1">Esteban Serrano</a>
          </p>
      ...
      {!firstLogin && (
        <div className="q_container">
          ...
          <button className="btn_check" onClick={checkAnswers}>
            ...
            <span class="front text"> {checkedAnswers === true ? "New Questions" : "Check Answers"}</span>
